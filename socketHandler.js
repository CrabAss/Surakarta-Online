...
module.exports = (io, app, next) => {
  ...
  hall.on('connection', socket => {
    User.findById(socket.handshake.session.userID).exec((error, user) => {
      ...
      if (user) {
        ...
        if (hallUserIDs.filter(e => e === socket.handshake.session.userID).length > 1) {
          ...
          if (hallUserIDs.length >= 2) {
            ...
            Game.newGame(hallUserIDs, (err, game) => {
    ...
    socket.on('disconnect', reason => console.log('Socket disconnected because of', reason))
  })
  ...
  game.on('connection', socket => {
    ...
    socket.on('ask_user', msg => {
      ...
      User.findById(socket.handshake.session.userID).exec((error, user) => {
        ...
        User.findById(msg.opponent).exec((error, user) => {
          if (error) return next(error)
          opponent = {
            username: user.displayName,
            gender: user.gender,
    ...
    socket.on('move', msg => {
      ...
      // gameBoard
      Game.findOne({ gameID: msg.gameID }, (err, currentGame) => {
        currentGame.moveStep(msg.oldPos, msg.newPos, msg.timeUsed, (err, gameBoard, oldPos, newPos, winner) => {
          if (err) {
          ...
          if (winner) {
            User.updateGameRecord(currentGame.gameID, currentGame.winner, currentGame.loser, err => {
    ...
    socket.on('surrender', msg => {
      ...
      // SEND BACK: END
      Game.findOne({ gameID: msg.gameID }, (err, currentGame) => {
        currentGame.surrenderGame(socket.handshake.session.userID, (err, winner) => {
          User.updateGameRecord(currentGame.gameID, currentGame.winner, currentGame.loser, err => {
            if (err) return next(err)
          })
          socket.to(msg.gameID).emit('end', { winner: winner })
