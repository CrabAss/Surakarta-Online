  ...
  if (isProfile) {
    if (valBirthYear()) {
      ...
      $.post('', $('#profile-form').serialize(),
        data => $('.alert-success').removeClass('d-none').text('Your profile has been updated successfully.')
      ).fail(xhr => $('.alert-danger').removeClass('d-none').text(xhr.responseText))
        .always(() => submitBtn.removeAttr('disabled').text('Save Changes'))
    }
  } else if (valPassword()) {
    submitBtn = $('#submit-credential')
    submitBtn.prop('disabled', true).text('Processing...')
    $.post('', $('#credential-form').serialize(), data => {
      ...
      .always(() => submitBtn.removeAttr('disabled').text('Update Password'))
  }
}

$(document).ready(() => {
  if (window.location.hash.substr(1) === 'credential') $('#credential-tab').tab('show')

  $('#submit-profile').click(() => valForm(true))
  $('#submit-credential').click(() => valForm(false))
})